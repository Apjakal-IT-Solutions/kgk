...
@frappe.whitelist()
def diagnose_excel_barcode_issue(file_url: str):
    """
    Detailed diagnosis of Excel barcode extraction issues
    """
    try:
        file_path = frappe.get_site_path(file_url.strip("/"))
        if not os.path.exists(file_path):
        ...
        print(f"=== EXCEL BARCODE DIAGNOSIS ===")
        ...
        # Check for barcode column variations
        barcode_variations = []
        for col in df.columns:
            if 'barcode' in col.lower():
                barcode_variations.append(col)
        
        print(f"Barcode column variations found: {barcode_variations}")
        
        # Check exact "Barcode" column
        if "Barcode" in df.columns:
            barcode_col = df["Barcode"]
            print(f"\n=== 'Barcode' Column Analysis ===")
            print(f"Total entries: {len(barcode_col)}")
            print(f"Non-null entries: {barcode_col.notna().sum()}")
            print(f"Null entries: {barcode_col.isna().sum()}")
            print(f"Unique non-null values: {barcode_col.dropna().nunique()}")
            
            # Sample values
            sample_values = barcode_col.dropna().head(10).tolist()
            ...
            print(f"ERROR: No 'Barcode' column found!")
        
        return {"success": True, "diagnosis_complete": True}
